...
describe("allowlisted data tools", () => {
  it("denies unknown roles and supports dry-run, apply, reconcile, and rollback export", async () => {
    ...
    expect((await allowed.importDryRun(source)).songs[0]?.action).toBe("insert");
    expect(database.sqlite.prepare("SELECT COUNT(*) AS count FROM songs").get()).toEqual({ count: 0 });
    expect((await allowed.importApply(source)).inserted).toBe(1);
    expect((await allowed.reconcile(source)).zeroDiff).toBe(true);
    expect((await allowed.rollbackExport()).songs).toContain("song-1");
    database.close();
  });
});
