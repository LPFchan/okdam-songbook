...
describe("songbook domain services", () => {
  it("fails closed when no role resolver is configured", async () => {
    const service = createSongbookService(database);
    await expect(service.createSong(allowed, songInput())).rejects.toThrow(/로그인 또는 허용된 계정/);
  });

  it("resolves the current role for each protected request", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    await expect(service.createSong(allowed, songInput())).resolves.not.toThrow();
    await expect(service.createSong({ email: "revoked@example.com" }, songInput({ tjNumber: "54321" }))).rejects.toThrow(DomainError);
    await expect(service.deleteSong({ email: "unknown@example.com" }, { id: "missing", expectedVersion: 1, clientRequestId: crypto.randomUUID() })).rejects.toThrow(/로그인 또는 허용된 계정/);
  });

  it("creates, searches, and returns anonymous public catalog rows", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    const created = await service.createSong(allowedPeer, songInput({ title: "フォニイ", titleReadingKo: "포니", artist: "ツミキ", tjNumber: "52537" }));
    expect((await service.catalog()).map((song) => song.id)).toEqual([created.id]);
    expect((await service.search("포니"))[0]?.id).toBe(created.id);
  });

  it("keeps favorites private to each signed-in account", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    ...
    expect(await service.setFavorite(allowed, request)).toEqual({ ownerSubject: "legacy-email:allowed@example.com", songId: created.id, favorite: true });
    expect(await service.setFavorite(allowed, request)).toEqual({ ownerSubject: "legacy-email:allowed@example.com", songId: created.id, favorite: true });
    expect(await service.favoriteSongIds(allowed)).toEqual([created.id]);
    expect(await service.favoriteSongIds(allowedPeer)).toEqual([]);
    expect(JSON.stringify(await service.catalog())).not.toContain(allowed.email);
    await service.setFavorite(allowed, { songId: created.id, favorite: false, clientRequestId: crypto.randomUUID() });
    expect(await service.favoriteSongIds(allowed)).toEqual([]);
  });

  it("rejects duplicates and reports version conflicts", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    const first = await service.createSong(allowed, songInput());
    expect(Object.hasOwn(first, "clientRequestId")).toBe(false);
    await expect(service.createSong(allowedPeer, songInput({ clientRequestId: crypto.randomUUID() }))).rejects.toThrow(DomainError);
    await expect(service.updateSong(allowedPeer, { id: first.id, expectedVersion: 99, clientRequestId: crypto.randomUUID(), title: "Changed" })).rejects.toThrow(/다른 곳/);
    const updated = await service.updateSong(allowedPeer, { id: first.id, expectedVersion: 1, clientRequestId: crypto.randomUUID(), title: "Changed" });
    expect(updated.title).toBe("Changed");
    expect(Object.hasOwn(updated, "clientRequestId")).toBe(false);
    expect(Object.hasOwn(updated, "expectedVersion")).toBe(false);
  });

  it("returns structured create outcomes, preserves TJ provenance, and hides deleted rows from lookup", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    const created = await service.createSongOutcome(allowed, songInput({ title: "Outcome", artist: "Artist", tjNumber: "77777" }));
    expect(created.outcome).toBe("created");
    const duplicate = await service.createSongOutcome(allowed, songInput({ title: "Outcome", artist: "Artist", tjNumber: "77777", clientRequestId: crypto.randomUUID() }));
    expect(duplicate).toMatchObject({ outcome: "duplicate", existing: { id: created.song?.id }, song: null, canOpen: true });

    const tj = await service.createTjSong(allowed, { tjNumber: "88888", title: "TJ source", artist: "TJ artist", lyricist: "", composer: "", sourceUrl: "https://tj.example/song" }, crypto.randomUUID());
  ...
  it("assigns a TJ-added song to the signed-in performer", async () => {
    ...
    const result = await service.createTjSong(allowed, {
      tjNumber: "88889",
  ...
  it("detects the country when adding a TJ song", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    ...
    const knownArtist = await service.createTjSong(allowed, {
      tjNumber: "22222",
    ...
    const korean = await service.createTjSong(allowed, {
      tjNumber: "33333",
      title: "좋은 날",
      artist: "아이유",
      lyricist: "",
  ...
  it("makes mutations replay-safe and audits the successful operation once", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    const clientRequestId = crypto.randomUUID();
    const first = await service.createSong(allowed, songInput({ clientRequestId }));
    const replay = await service.createSong(allowed, songInput({ clientRequestId }));
  ...
  it("keeps update results and their stored and replayed responses free of request fields", async () => {
    const service = createSongbookService(database, { roleResolver: roleResolver() });
    const created = await service.createSong(allowed, songInput());
    const clientRequestId = crypto.randomUUID();
  ...
  it("creates, counts, and cancels performance records", async () => {
    ...
    expect(await service.performanceStats(created.id)).toEqual({ count: 0, lastPerformedAt: "" });
  });

  it("maps domain-only errors to the committed shared API vocabulary", async () => {
